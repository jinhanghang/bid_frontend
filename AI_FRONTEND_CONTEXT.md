...
> 最新口径：`/prompt-template` 模块已删除；大数据列表/左侧列表必须首屏小分页、滚动追加或底部分页；企业下拉不得全量加载，必须远程分页搜索；AI文档、AI标书技术方案导出优先走异步导出任务；SSE/轮询需要支持取消、退避和页面隐藏暂停；AI文档、AI标书技术方案不默认选择 AI 等级，必须由用户显式选择。
...
### 0.0.2.9 第五步 AI 主流程稳定性前端口径

- 内部生成引擎兼容 API 与 AI 标书技术方案页面已增加 `visibilitychange` 恢复逻辑：页面从隐藏恢复可见时，立即补查全局 AI 任务、全文生成任务、解析任务和目录生成任务状态。
- 页面隐藏时仍允许轮询函数跳过请求，避免后台标签页产生无意义请求；恢复可见后必须主动触发一次状态查询，避免界面卡在“生成中 / 导出中 / 目录生成中”。
- 历史生成成果导出和 AI 标书技术方案导出已增加 `notifyRequestError` 兜底；异步导出任务失败、超时或下载失败时必须给用户安全提示。
- 生成任务 `task.message` 展示前必须走 `normalizeStreamErrorMessage`，禁止直接展示第三方错误、Java Exception、SQL、HTML、request_id、Connection reset 等原始内容。
- 本次不修改按钮布局、路由菜单、AI 生成算法、知识库问答主流程、额度扣减逻辑和任何被排除模块。
...
- AI标书、AI文档页面都不能给 `aiLevel` 写默认值，必须由用户显式选择。
...
- AI文档、AI标书技术方案新建草稿时 `aiLevel` 保持空值，页面生成/上传前必须提醒用户先选择 AI 等级。
- 普通业务页面不展示“服务商/模型名称”等模型明细，只展示用户需要选择和理解的 AI 等级、场景等业务信息。
- AI复审抽屉不使用全屏遮罩锁定页面，历史记录查询走后端统一前缀接口 `/ai_bid/api/ai-review-record`。
- 内部生成引擎知识库选择需要在“节点编辑、整体生成、切换步骤/刷新详情”之间尽量保留用户已选状态，不能因详情回填为空就清掉当前选择。
...
- 内部生成引擎、AI文档、AI标书技术方案的原始模型错误、堆栈、超长异常，前端统一显示：`AI任务执行失败，请稍后重试或检查模型配置/额度`。
...
- 内部生成引擎、AI标书技术方案的单章节生成和 AI帮写方向使用 `fetch + ReadableStream` 处理 SSE。
...
| 路径 | 页面 | 说明 |
|---|---|---|
| `/ai/solutions` | redirect `/ai-bid` | 历史兼容跳转，独立产品入口已下线 |
...
| `api/aiSolution.js` | 内部生成引擎兼容 API，供 AI文档/AI标书技术方案复用；不再作为独立 独立生成成果页面（已下线）入口 |
...
### 9.1 内部生成引擎兼容 API（无独立页面）

文件：`src/api/aiSolution.js`

当前口径：

- 独立生成成果页面已下线，当前上传包不应包含 `src/views/ai/solutions/index.vue`（已删除，禁止恢复）。
- `/ai/solutions` 仅作为历史兼容路由，必须继续跳转 `/ai-bid`。
- `src/api/aiSolution.js` 继续保留，作为 AI文档和 AI标书技术方案复用内部生成引擎的兼容 API。
- 后续不得恢复独立产品菜单、页面、下载中心筛选或回收站类型。
- 前端注释需要描述底层能力时，统一使用“内部生成引擎”“生成成果”“技术方案目录树”等口径。
...
- 日志和 UI 文案必须写“AI标书技术方案”，不能写成“内部生成引擎”。
...
## 0.18 2026-06-03 内部生成引擎 / AI文档 / AI标书技术方案统一检查与审稿入口

本次同步 内部生成引擎、AI文档、AI标书技术方案的生成后检查、审稿和导出口径。后续前端修改时不要只改 独立生成成果页面（已下线）。
...
## 2026-06-15 AI目录生成状态错乱说明
页面“已报错但后台仍在跑”的根因在后端超时兜底扫描误判，不是前端按钮或轮询逻辑本身。后端已通过目录生成心跳和超时扫描二次校验修复；前端继续按任务状态轮询展示。

## 2026-06-15 AI标书左侧状态同步修复
- 问题：AI标书技术方案中间区域已进入“正在生成技术方案目录”，但左侧项目卡片和技术方案 doc 标签仍取项目列表旧 `technicalStatus`，可能继续显示“待制作”。
- 修复：左侧项目摘要和展开文档标签优先使用当前页面运行态、目录 pending 状态和 `technicalSolution.status` 计算显示状态；目录生成中显示“目录生成中”，目录生成完成后显示“目录已生成”。
- 边界：本次只修复前端状态展示，不修改后端生成逻辑、模型调用、目录结构、额度扣减和接口路径。

## 2026-06-15 热修复：技术方案字数弹窗触发时机
- AI 标书技术方案目录生成完成后，不再自动弹出字数设置弹窗，避免用户刚进入目录预览时被强制打断。
- 当用户点击底部“开始生成”且末级章节目标字数仍为 0 时，再弹出字数设置弹窗；确认后继续进入全文生成设置。
- 当用户点击单个末级目录行“生成”且该章节目标字数仍为 0 时，先弹出字数设置弹窗；确认后继续打开单章节生成弹窗。
- 当前交互口径：目录生成只负责生成结构，正文生成入口负责补齐篇幅设置。

## 2026-06-15 热修复：技术方案生成中目录选中态保持
- 问题：AI 标书技术方案全文生成中，前端轮询刷新目录后会自动切换到正在生成或最后生成的章节，覆盖用户手动点击的章节，表现为“选中后一会选中状态就没了”。
- 修复：用户手动点击目录叶子节点后，前端记录手动选中章节，后续轮询刷新只同步该章节最新数据，不再自动跳转到其他章节；只有用户未手动选中过章节时，才自动跟随正在生成或最后生成的章节。
- 展示：`OutlineTree` 新增 `selectedId`，生成模式下当前章节持续高亮。
- 范围：只改 AI 标书技术方案前端选中态和预览跟随逻辑，不改后端、数据库、模型调用、额度扣减和接口路径。

### 2026-06-15 性能优化第一步：轮询合并与防重复请求
- 内部生成引擎兼容 API 与 AI 标书技术方案页面已对全局运行任务查询做请求合并和最小间隔限制，避免 mounted、visibilitychange、生成轮询同时触发重复请求。
- 全局运行任务轮询周期从 5 秒调整为 10 秒；页面恢复可见时仍强制补查一次，保证状态恢复能力不被削弱。
- AI 标书技术方案页面对技术方案详情加载、工作流刷新、目录生成状态查询增加 in-flight 复用/并发锁，避免生成中重复刷新目录树和左侧状态。
- 本次不改路由、不改按钮布局、不改接口路径、不改 AI 生成、知识库、额度扣减和已排除模块。

### 0.0.2.11 性能优化第二步：AI标书目录树渲染收口

- AI 标书技术方案生成中，目录树刷新不能再每轮直接替换整棵 `technicalOutlines`，应按节点 ID 做原位合并，保留节点对象引用和当前选中章节，减少目录树重绘、右侧预览抖动和用户选中态异常。
- 详情轮询返回后先做目录树签名比较；目录结构、生成状态、目标字数、实际字数和正文摘要无变化时跳过树同步。
- OutlineTree / WritingDirectionEditor 内部递归节点必须带稳定 key，避免大目录下 Vue 按索引 diff 导致 DOM 重排压力过大。
- 本次不改变按钮交互、字数弹窗触发时机、生成接口、任务轮询口径和任何后端业务逻辑。

### 0.0.2.12 性能优化第三步：后端状态查询与日志降噪配套口径

- 前端无需因本次后端降噪修改接口调用方式；内部生成引擎、AI 标书技术方案仍按第一步优化后的 10 秒轮询、页面隐藏暂停、恢复可见补查一次执行。
- 高频状态查询接口在后端正常 200 且低于慢请求阈值时不再产生普通 access 明细和查询摘要 operation 日志；异常和慢请求仍保留，前端错误提示和状态恢复口径不变。
- 本次不改前端路由、按钮交互、字数弹窗、章节选中态、目录树渲染逻辑和任何被排除模块。

### 0.0.2.13 性能优化第四步：数据库索引配套口径

- 第四步为后端 MySQL 索引补丁，前端无需改接口调用、路由、按钮、轮询周期、目录树合并逻辑和章节选中态逻辑。
- 前端验收重点保持：企业下拉、AI 标书项目列表、知识库列表、技术方案生成中目录刷新和任务状态恢复仍正常；页面行为不因数据库索引补丁发生变化。
- 如果索引执行后列表仍慢，下一步应结合浏览器 Network、后端慢查询摘要和 MySQL `EXPLAIN` 定位具体 SQL，而不是把前端恢复为全量加载或提高 pageSize。

## 2026-06-15 AI 长任务队列和并发控制第一步前端口径

- 本步骤不改前端页面代码，只同步交互口径：后端已增加 AI 长任务入队前的任务锁和并发限制。
- 前端遇到“已有目录生成任务正在执行”“已有正文生成任务正在执行”“当前章节已有生成任务正在执行”“当前企业AI任务较多”等提示时，应保持当前页面状态并继续通过既有轮询恢复任务状态，不要重复弹窗或自动重发请求。
- 生成按钮仍按现有 loading / disabled / 轮询逻辑执行；后续如继续做第二步，可再补充任务排队、取消任务、队列位置等页面展示。

### 2026-06-15 AI 长任务队列和并发控制第二步前端口径

- AI 标书技术方案页新增运行态状态卡片：排队中、执行中、其他 AI 任务占用分别展示不同提示。
- 技术方案生成中可显示任务类型、任务进度和安全化任务消息；用户可点击“取消任务”，前端调用 `POST /bid-project/{id}/technical-solution/task/{taskId}/cancel`。
- 取消后前端清理本地 pending task、停止轮询、刷新技术方案详情和工作流状态；若模型调用已发出，后端会在当前模型调用返回后停止，页面文案必须避免承诺“立即强杀”。
- 并发冲突仍以后端业务提示为准，前端不自行绕过用户/企业/项目/方案互斥规则。

### 2026-06-15 AI 长任务队列和并发控制第三步前端口径

- 历史独立生成成果页面已下线；该页面的任务状态卡片不再作为当前前端验收项。
- 内部生成引擎上传解析区域在解析任务 WAITING/PARSING/EXTRACTING 时显示“取消解析”按钮，取消后停止本地解析轮询并刷新当前方案状态。
- 取消任务不是立即强杀模型或解析 HTTP 调用；前端提示必须保持“后台会在当前步骤结束后的检查点停止”的口径，避免承诺立即中断。
- 并发冲突、企业任务过载、已有任务执行中等提示继续以后端业务提示为准，前端不自动重发请求、不绕过任务锁。

## 2026-06-15 AI长任务队列与并发控制第四步：任务中心联动
- AI任务中心新增取消任务能力：对后端返回 `cancelable=true` 的解析/生成运行态任务展示“取消”按钮，调用 `cancelAiTask(taskCategory, taskId)`。
- 取消前弹出确认说明：取消会在后台安全检查点停止，已发出的模型调用不会被强制中断；取消成功后刷新任务列表。
- AI任务中心新增“已取消”状态筛选，状态展示支持 `CANCELED`。
- 页面支持运行态自动刷新：存在 WAITING/PENDING/RUNNING/PARSING/EXTRACTING 任务时每 8 秒刷新一次；浏览器标签隐藏时暂停，恢复可见时补查一次。
- 导出任务和知识库问答任务暂不显示可取消按钮，只展示后端 `cancelTip`；不要在前端绕过后端规则自行强制取消。

## 2026-06-16 知识库文件状态标签修复

- 问题：后端重新入库处理中或成功后，如果历史 `errorMsg` 尚未清理，前端会继续在文件状态列展示错误标签，造成“解析成功但索引异常”的误判。
- 修复：知识库文件状态提示只在非处理中状态展示；`errorMsg` 统一 trim 后判断，处理中不展示历史错误标签。真正的成功索引异常仍由后端 `parseStatus=2 / embeddingStatus=2 / errorMsg 非空` 表达。
- 范围：只改 `src/views/knowledge/base/index.vue` 文件状态标签展示，不改知识库问答、检索、上传和文件列表分页逻辑。

## 0.0.3 2026-06-29 资料库图片库与技术方案插图前端口径

- 资料库入口调整为三个页签：企业资料、图片库、知识库；旧 `/knowledge/bases` 路由保留兼容并重定向到 `/materials/knowledge`。
- 新增图片库页面 `src/views/material/image/index.vue`，支持图片上传、分页查询、预览、编辑、删除和复制插图标记。
- AI标书技术方案章节预览从纯文本展示改为 `SectionContentPreview`，支持渲染 `aibid-file://` 图片标记；图片预览通过 `/files/download/{fileId}` 读取 blob，不直接使用 OSS URL。
- AI标书技术方案章节编辑区新增“插入图片”，从图片库选择图片并写入 Markdown 标记；用户仍需点击“保存”后才会写入章节正文。
- 本次不改 AI 生成逻辑、不改知识库检索流程、不让图片参与模型调用。

## 0.0.2.14 图片库插图第二阶段增强口径

- 图片库分类、适用章节、标签增加前端固定选项，减少后续图片素材失控；上传/编辑仍允许手动补充自定义值，避免历史数据或特殊行业场景无法录入。
- 技术方案章节插图弹窗接收当前章节标题，根据标题关键词推荐图片分类和适用章节，例如安全文明、质量管理、进度计划、施工部署、项目概述等。
- 插图弹窗打开时会优先按推荐条件筛选启用图片；推荐条件无结果时自动回退展示全部启用图片，并给出提示，避免用户误以为图片库为空。
- 图片卡片增加“推荐”标识，推荐图片排序靠前；插入设置仍支持图片说明、宽度和对齐方式。
- 本阶段只改前端，不改后端接口、不改数据库、不影响 Word/PDF 导出标记格式。

## 0.0.2.13 2026-06-29 图片库引用统计与删除保护前端口径

- 图片库保持单库结构，不拆成多个图库；页面继续通过分类、标签、适用章节、状态筛选管理图片素材。
- 图片库卡片展示引用次数：当 `referenceCount > 0` 时显示“引用 N 次”。
- 图片库新增“引用”按钮，调用 `GET /material-images/{id}/references` 查看图片被哪些技术方案章节引用。
- 删除图片前先查询引用统计；如果图片已被章节引用，前端不直接删除，弹出引用明细并提示需要先到章节正文中删除图片引用。
- 删除保护不删除章节正文引用，也不删除图片库原图；只能阻止误删导致 Word/PDF 导出图片不可用。

## 0.0.2.13 2026-06-30 图片库页面与插图弹窗体验优化

- 本次限定前端体验优化，不改后端接口、不改数据库、不改图片库拆分结构。
- 图片库继续保持单一素材库结构，通过分类、标签、适用章节、状态和引用次数管理图片。
- 图片库页面新增顶部说明、当前筛选统计、本页已引用/可删除统计、状态筛选、重置筛选、图片大图预览、空数据上传引导。
- 图片卡片优化：预览遮罩、启用/停用与引用标签置顶展示、标签拆分展示、已引用图片删除按钮置灰，并提示需先删除章节引用。
- 上传/编辑弹窗改为更清晰的分组布局，字段仍沿用现有接口：图片名称、分类、标签、适用章节、适用场景、图片说明。
- 技术方案插图弹窗优化：展示当前章节、推荐条件、全部/推荐切换、右侧选中图片预览和插入设置，仍使用原有 `aibid-file://` 图片标记。
- 本次不影响 Word/PDF 导出、不影响 AI 额度、不影响知识库检索、不影响图片引用统计和删除保护接口。


## 2026-06-30 AI方案产品入口彻底下线口径

- 产品侧只保留 AI文档、AI标书，不再展示独立产品菜单、页面、下载中心筛选和回收站类型。
- `/ai/solutions` 旧路由保留兼容跳转到 `/ai-bid`，避免旧链接白屏。
- 当前上传包已删除 `src/views/ai/solutions/index.vue`，覆盖补丁时禁止重新引入该页面。
- `src/api/aiSolution.js` 继续保留，作为内部生成引擎兼容 API，供 AI文档和 AI标书技术方案复用，不作为产品入口。
- 历史 `ai_solution` 下载记录和 `AI_SOLUTION` 回收站数据在前端统一显示为“历史生成成果”。
- 后续新增文案禁止再恢复独立产品入口口径；需要描述底层能力时使用“内部生成引擎”或“生成成果”。


## 2026-06-30 项目基线整理补丁

- 本次只清理前端文档、注释和兼容 API 说明，不新增页面，不恢复独立产品入口。
- 当前产品入口仍为 AI文档、AI任务、AI标书、资料库。
- `/ai/solutions` 保留历史兼容跳转 `/ai-bid`；`src/api/aiSolution.js` 保留为内部生成引擎兼容 API。
- 不改接口路径、请求参数、返回结构、生成逻辑、图片库、导出和额度逻辑。

## 0.0.2.11 2026-06-30 正文章节受控并行生成前端口径

- 本次第一版不修改前端页面、路由、API 封装和接口参数。
- AI文档与 AI标书技术方案继续按原流程提交全文生成任务并轮询任务详情。
- 后端内部会把同一全文任务拆成末级章节受控并行执行；前端仍只看到同一个 taskId、同一套 `progress / finishedNodes / failedNodes / status / message`。
- 前端验收重点不变：生成中状态、取消状态、PARTIAL 失败提示、页面隐藏恢复轮询、Word/PDF 导出按钮状态不能回退。
- 如后续要增强体验，再单独补 AI文档取消按钮、失败章节列表、只重试失败章节等能力；本次不做。

## 2026-06-30 并行生成后续补丁口径

- 已补齐 AI文档全文生成任务取消入口：`POST /ai-document/task/{taskId}/cancel`，用于产品层取消 AI文档生成任务，底层仍复用内部生成引擎取消逻辑。
- AI文档前端生成栏增加取消按钮；取消不是强杀已经发出的模型请求，而是在模型返回后的检查点停止继续保存和提交新章节。
- 本次不改数据库、不改接口入参/出参、不改 AI标书技术方案已有取消接口、不改导出和图片库。


## 0.0.2.14 2026-06-30 失败章节补跑前端口径

- AI文档页面新增失败/未完成章节提示和“重试失败章节”按钮。
- AI标书技术方案页面新增“重试失败章节”按钮，并在任务类型文案中识别 `RETRY_FAILED`。
- 前端不新增路由，不改菜单，不恢复独立 AI方案入口。
- 补跑请求复用原全文生成接口，请求体携带 `retryFailedOnly: true`；后端负责跳过已成功章节。
- 新文档未开始生成时不展示失败章节补跑按钮；仅在已有部分正文或后端状态进入部分完成/失败残留时展示。
- 补跑完成后继续沿用原任务轮询、取消、详情刷新、导出可用性判断。

## 0.0.2.14 2026-06-30 AI任务中心详情与章节补跑前端口径

- AI任务中心新增“详情”抽屉，用于查看任务基础信息、失败原因、失败/未完成章节列表。
- 生成任务详情支持“重试全部失败章节”和“重试本章”。
- “重试全部失败章节”只传 `retryFailedOnly=true`，不覆盖已成功章节。
- “重试本章”复用内部生成引擎兼容 API 的单章节流式生成能力，只覆盖当前章节正文。
- 任务详情中的失败原因继续使用 `normalizeStreamErrorMessage` 兜底脱敏，禁止展示 Java Exception、SQL、第三方 request_id、HTML、accessKey、token 等原始错误。
