...
- AI 方案页面与 AI 标书技术方案页面已增加 `visibilitychange` 恢复逻辑：页面从隐藏恢复可见时，立即补查全局 AI 任务、全文生成任务、解析任务和目录生成任务状态。
- 页面隐藏时仍允许轮询函数跳过请求，避免后台标签页产生无意义请求；恢复可见后必须主动触发一次状态查询，避免界面卡在“生成中 / 导出中 / 目录生成中”。
- AI 方案导出和 AI 标书技术方案导出已增加 `notifyRequestError` 兜底；异步导出任务失败、超时或下载失败时必须给用户安全提示。
...
- AI 方案页面与 AI 标书技术方案页面已对全局运行任务查询做请求合并和最小间隔限制，避免 mounted、visibilitychange、生成轮询同时触发重复请求。
...
- 前端无需因本次后端降噪修改接口调用方式；AI 方案、AI 标书技术方案仍按第一步优化后的 10 秒轮询、页面隐藏暂停、恢复可见补查一次执行。
...
- AI 方案页面新增生成任务状态卡片，展示排队中、执行中、其他 AI 任务占用等状态，并支持取消当前 WAITING/RUNNING 生成任务。
- AI 方案上传解析区域在解析任务 WAITING/PARSING/EXTRACTING 时显示“取消解析”按钮，取消后停止本地解析轮询并刷新当前方案状态。
