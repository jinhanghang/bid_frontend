...
> 最新口径：`/prompt-template` 模块已删除；大数据列表/左侧列表必须首屏小分页、滚动追加或底部分页；企业下拉不得全量加载，必须远程分页搜索；AI方案、AI文档、AI标书技术方案导出优先走异步导出任务；SSE/轮询需要支持取消、退避和页面隐藏暂停；AI方案、AI文档、AI标书技术方案不默认选择 AI 等级，必须由用户显式选择。知识库问答页面需要识别后端低置信度/无证据结果，不应把无资料提示当成普通成功生成内容。
...
### 0.0.1 2026-06-05 RAG 商用化交互口径

- AI方案、AI文档、AI标书技术方案的 `aiLevel` 初始值必须保持为空；上传、解析、生成目录、生成全文、生成技术方案前必须校验用户已显式选择 AI 等级。
- 前端不能再用 `BASIC`、`STANDARD` 或任一档位作为隐藏默认值；后端返回 `AI_LEVEL_REQUIRED` 时，应提示用户先选择 AI 等级。
- 知识库问答接口如果返回低置信度、无证据或 `KNOWLEDGE_NO_EVIDENCE` 类错误，页面应展示“当前知识库未检索到相关资料”的业务提示，不要渲染成普通 AI 答案。
- RAG 问答结果应保留后端返回的引用来源、证据数量等字段；后续页面展示时优先显示文件名、片段摘要、资料编号，便于用户核对来源。
- 检索日志和前端调试输出不得打印完整知识片段、企业资料正文、合同正文、报价正文等敏感内容。
- 知识库入库状态以前端轮询到的后端聚合状态为准；存在文件处理中或失败时，不要仅因某个文件成功就把知识库整体显示为成功。

### 0.0.2 2026-06-05 RAG 正式商用增强交互口径

- 知识库问答请求必须携带 `requestId`，用于后端幂等，避免重复点击导致重复调用模型和重复扣额度。
- 知识问答页面需要展示低置信度提示、证据数量、文件名、页码/章节、工作表、幻灯片、切片序号、检索来源和摘要。
- 前端应优先展示后端返回的 `citation`；没有 `citation` 时再用 `fileName/sourceRef/pageNo/sectionTitle/sheetName/slideNo` 组合展示。
- 知识库问答低置信度结果不能渲染成普通成功回答，应有明确 warning 状态。
- 前端调试、控制台输出、日志上报不得打印完整引用片段正文，只能展示摘要或由用户主动展开页面内内容。


### 0.0.3 2026-06-05 目录生成锁定、篇幅弹窗与知识库选择传递口径

- AI方案、AI文档、AI标书技术方案点击“生成目录/生成大纲”后，页面相关操作必须进入 busy/disabled 状态，避免重复点击、继续编辑或切换配置造成目录和章节状态错乱。
- AI文档生成目录后必须弹出“章节字数/篇幅设置”弹窗，让用户选择固定字数或自动控制；不能静默跳过该弹窗。
- AI标书技术方案“初步解析/生成目录”阶段选择的知识库必须保留到后续目录生成和全文生成流程，不能在生成目录后丢失。
- AI方案生成目录时，如果后端返回的目录节点没有知识库字段，前端应使用当前目录表单已选知识库回填到节点，后续整体生成继续带上这些知识库。
- 知识库选择的优先级：当前表单显式选择 > 已保存方案/项目配置 > 节点自身配置；禁止因为详情接口暂时为空就清空用户刚选的知识库。


### 0.0.6 2026-06-05 企业申请 / 公司审批流程修复口径

- 前端不再弹出“完善企业信息”强制窗口，也不再调用 `/auth/complete-enterprise`。
- 普通用户未绑定企业时，首页只引导进入 `/system/enterprise-apply` 提交企业申请。
- 公司审批入口保留给超级管理员、平台管理员、企业管理员，用于审核注册新企业和加入已有企业申请。
- 未绑定企业用户访问首页时不再调用业务统计接口，避免无企业状态下触发无意义的业务接口错误。
...
## RAG 商用化整改补充规则（2026-06）

- AI方案、AI文档、AI标书技术方案均不得默认 BASIC；新建草稿可以为空，但上传、解析、生成目录、生成正文、单章节生成、帮写编写方向前必须显式选择或已有方案保存 AI 等级。
- AI标书技术方案复用 AI方案引擎时，前端传参优先使用当前表单 AI 等级，缺省时回退内部方案已保存 AI 等级，避免页面刷新/旧数据导致误判。
- 知识库问答无命中资料时必须停止生成，不调用大模型、不扣额度，前端应展示低置信度/无依据提示。
- RAG 回答需要保留引用来源，后端会补充 `[资料N]` 来源摘要；后续前端可基于 `references` 展示文件名和切片序号。
- 检索日志不得保存完整知识片段，只允许保存 chunkId、fileName、score、retrievalSource 和短摘要。
- 知识入库失败状态必须在主事务回滚后另起事务写入，避免解析/向量失败后状态仍停留在处理中或旧状态。
- `application-local.yml` 本次按用户要求暂不处理；后续如做生产安全整改，再统一改环境变量/配置中心。

## 2026-06-05 商用增强 v5：前端交互规则

1. 知识库页面新增“重建索引”按钮，只对可管理知识库的账号展示。
2. 知识库文件列表新增“解析质量”列，展示 HIGH/MEDIUM/LOW 和评分；tooltip 展示解析质量报告建议。
3. AI方案生成目录 / 生成全文前必须先调用生成前检查接口，不通过则弹出检查项并阻止继续生成。
4. AI文档生成目录 / 生成全文前同样调用生成前检查接口，避免资料不完整时直接进入长任务。
5. 章节字数弹窗仍保留，生成目录完成后进入篇幅设置，不允许静默跳过。
6. 原有知识库选择结果仍必须贯穿目录生成、章节生成和全文生成。

## 2026-06-05 V6 前端商用稳定增强（不包含 OCR）
- 新增 `/ai/tasks` AI任务中心页面，统一查看资料解析、AI生成、导出任务状态。
- AI方案和 AI文档导出前会先调用服务端导出检查；错误项未通过时阻止导出，警告项允许用户确认后继续。
- 知识库问答结果下方增加反馈按钮：有用、无用、引用不准、答案不完整；反馈会提交给后端做质量统计。
- 本轮不改变原有 AI标书、AI方案、AI文档主流程，不加入 OCR 前端入口。
