    ...
    <main v-if="current" class="conversation-panel">
      <header class="conversation-head">
        ...
        <div class="head-actions">
          ...
          <el-tag class="model-tag" type="success" :title="modelDisplayName(current)">{{ modelDisplayName(current) }}</el-tag>
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="生成模型" required>
          <el-select v-model="newForm.modelConfigId" style="width:100%" placeholder="请选择已配置的模型" :loading="loadingModels">
            <el-option v-for="item in models" :key="item.id" :label="item.displayName || item.modelName" :value="item.id">
...
function modelDisplayName(value) { const item = models.value.find(model => model.id === value?.modelConfigId); return item?.displayName || value?.modelName || providerName(value?.modelProvider || value?.provider) || '系统默认模型' }
