    ...
    <main v-if="current" class="conversation-panel">
      <header class="conversation-head">
        ...
        <div class="head-actions">
          ...
          <el-button @click="router.push('/ai-bid/workbench')">专业工作台</el-button>
        </div>
      </header>
      ...
      <section ref="messageBox" class="messages">
        ...
        <article v-for="m in messages" :key="m.id" class="message" :class="m.role">
          <div class="avatar">{{ m.role==='user'?'我':'AI' }}</div><div class="bubble-wrap">
            ...
            <div class="bubble"><div v-if="m.status==='GENERATING'&&!m.content" class="stream-status"><span>{{ m.stageText||'正在准备' }}</span><span>{{ m.progress||0 }}%</span></div>
              <el-progress v-if="m.status==='GENERATING'&&!m.content" :percentage="m.progress||0" :show-text="false" :stroke-width="4" />{{ m.content }}<i v-if="m.status==='GENERATING'" />
            </div>
            <div v-if="m.role==='assistant'&&m.status!=='GENERATING'" class="message-actions">
              ...
              <el-button link type="primary" :loading="adoptingId===m.id" @click="adopt(m)">采用到标书</el-button>
            </div>
          </div>
        </article>
      </section>
...
import { createConversation, getConversationMessages, getConversationRun, listConversations, regenerateConversationMessage, resumeConversationRun, stopConversationRun, streamConversationMessage, uploadConversationAttachment } from '@/api/aiConversation'

const router=useRouter(), route=useRoute(), projects=ref([]), current=ref(null), keyword=ref(''), loadingProjects=ref(false)
...
const createForm=reactive({noticeId:'',projectName:'',projectType:'',file:null})
...
onMounted(()=>loadProjects(route.query.projectId)); onBeforeUnmount(()=>{clearTimeout(timer);stopPolling()})
...
function openCreate(){createDialog.value=true;createTab.value='notice';loadNotices()}
...
async function createProject(){creating.value=true;try{let id;if(createTab.value==='notice'){if(!createForm.noticeId)return ElMessage.warning('请选择标讯商机');id=await createBidProjectFromNotice(createForm.noticeId)}else if(createTab.value==='upload'){if(!createForm.file)return ElMessage.warning('请选择招标文件');const fd=new FormData();fd.append('file',createForm.file);const w=await uploadTenderProject(fd);id=w?.project?.id||w?.id}else{if(!createForm.projectName.trim())return ElMessage.warning('请输入项目名称');id=await createBidProject({projectName:createForm.projectName,projectType:createForm.projectType})}createDialog.value=false;Object.assign(createForm,{noticeId:'',projectName:'',projectType:'',file:null});await loadProjects(id);ElMessage.success('AI标书项目已创建')}finally{creating.value=false}}
