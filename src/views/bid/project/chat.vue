<template>
  <div class="bid-chat-page" :class="{ 'project-list-collapsed': projectListCollapsed && current }">
    <aside v-show="!projectListCollapsed || !current" class="project-list">
      <div class="side-head"><div><h2>AI标书</h2><p>对话编制 · 资料驱动</p></div><div class="side-actions"><el-button link type="primary" @click="openProfessionalWorkbench">工作台</el-button><el-button type="primary" :icon="Plus" circle @click="openCreate" /></div></div>
    ...
    <main v-if="current" class="conversation-panel">
      <header class="conversation-head">
        ...
        <div class="head-actions">
          <el-button type="primary" plain @click="openProfessionalWorkbench">专业工作台</el-button>
...
async function loadProjects(selectId){loadingProjects.value=true;try{const p=await pageBidProjects({pageNum:1,pageSize:100,keyword:keyword.value||undefined});projects.value=p?.records||[];const target=selectId?projects.value.find(x=>String(x.id)===String(selectId)):null;if(target)await selectProject(target)}finally{loadingProjects.value=false}}
...
function openProfessionalWorkbench(){
  setAiBidFocus(false)
  router.push({path:'/ai-bid/workbench',query:current.value?.id?{projectId:current.value.id}:undefined})
}
