...
import { batchUpdateBidProjectTechnicalAllOutlineWordCount } from '@/api/technicalWordCount'
import {
...
const TECH_GENERATION_TIMING_KEY = 'ai_bid_technical_generation_timing'

const technicalGenerationClock = ref(Date.now())

let technicalGenerationClockTimer = null

const technicalGenerationTiming = reactive({
  projectId: '',
  taskId: '',
  taskType: '',
  status: '',
...
const technicalAllWordCount = ref('')
const technicalAllWordUpdating = ref(false)
// 目标字数属于章节配置，已生成章节也允许调整；调整后由用户决定是否重新生成。
const technicalEditableLeafCount = computed(() => technicalLeafNodes.value.length)

function normalizeTargetWordCount(value) {
  if (value === 'AUTO' || value === '' || value === null || value === undefined) return null
...
const technicalHasFlexibleWordCount = computed(() => technicalLeafNodes.value.some((node) => {
  const target = node?.targetWordCount ?? node?.wordCount ?? node?.section?.targetWordCount
...
const technicalGenerationTimingVisible = computed(() => {
  return Number(technicalGenerationTiming.startedAt || 0) > 0
    && String(technicalGenerationTiming.projectId || '') === String(selectedProject.value?.id || '')
})

const technicalGenerationTimingActive = computed(() => {
  return technicalGenerationTimingVisible.value && isGenerationTimingActiveStatus(technicalGenerationTiming.status)
})

const isCurrentTechnicalContentTimingActive = computed(() => {
  return technicalGenerationTimingActive.value
    && String(technicalGenerationTiming.taskType || '').toUpperCase() !== 'OUTLINE_GENERATE'
})

const technicalGenerationElapsedMs = computed(() => {
  if (!technicalGenerationTimingVisible.value) return 0
  const startedAt = Number(technicalGenerationTiming.startedAt || 0)
  const finishedAt = Number(technicalGenerationTiming.finishedAt || 0)
  const endAt = finishedAt > 0 ? finishedAt : technicalGenerationClock.value
  return Math.max(0, endAt - startedAt)
})

const technicalGenerationElapsedText = computed(() => formatGenerationDuration(technicalGenerationElapsedMs.value))

const technicalGenerationTimingLabel = computed(() => technicalTaskTypeLabel(technicalGenerationTiming.taskType))

const technicalGenerationTimingStatusText = computed(() => {
  const status = String(technicalGenerationTiming.status || '').toUpperCase()
  if (isGenerationTimingActiveStatus(status)) return status === 'WAITING' ? '排队及生成实时用时' : '生成实时用时'
  if (status === 'SUCCESS' || status === 'PARTIAL') return '本次总用时'
  if (status === 'CANCELED') return '取消前用时'
  if (status === 'FAILED' || status === 'TIMEOUT') return '失败前用时'
  return '最近一次用时'
})
...
onMounted(async () => {
  ...
  restoreTechnicalGenerationTiming()
  technicalGenerationClock.value = Date.now()
  technicalGenerationClockTimer = window.setInterval(() => {
    technicalGenerationClock.value = Date.now()
  }, 1000)
...
onBeforeUnmount(() => {
  ...
  if (technicalGenerationClockTimer) window.clearInterval(technicalGenerationClockTimer)
  technicalGenerationClockTimer = null
...
async function loadTechnicalSolution(options = {}) {
  ...
  technicalSolutionRequestPromise = (async () => {
    try {
      ...
      hydrateTechnicalOutlinesFromSolution(technicalSolution.value)
      syncTechnicalGenerationStateFromSolution(solution, projectId)
...
async function generateTechnicalOutline() {
  ...
  technicalGeneratingOutline.value = true
  beginTechnicalGenerationTiming(projectId, 'OUTLINE_GENERATE')
  ...
  try {
    ...
    if (String(selectedProject.value?.id || '') === String(projectId)) {
      ...
      if (technicalOutlines.value.length) {
        ...
        finishTechnicalGenerationTiming({ projectId, taskType: 'OUTLINE_GENERATE', status: 'SUCCESS' })
        clearTechnicalOutlinePending(projectId)
        const elapsedText = technicalGenerationElapsedText.value
        if (needWordPreset) {
          resetWordPresetSelection()
          wordPresetVisible.value = true
          ElMessage.success(`技术方案目录已生成，本次用时 ${elapsedText}，请继续设置篇幅`)
        } else {
          wordPresetVisible.value = false
          ElMessage.success(`长篇技术方案目录已生成，本次用时 ${elapsedText}，系统已按目标总字数分配章节篇幅`)
        }
      } else {
    ...
    // 不管当前是否还停留在技术方案页，都启动一次结果检测；目录已在本次响应中返回时不重复弹出成功提示。
    await checkTechnicalOutlineReady(projectId, technicalOutlines.value.length > 0)
  } catch (e) {
    finishTechnicalGenerationTiming({ projectId, taskType: 'OUTLINE_GENERATE', status: 'FAILED' })
    clearTechnicalOutlinePending(projectId)
    ...
    if (outlineRequestAccepted && !technicalOutlines.value.length && technicalOutlinePendingProjectId.value) {
      ...
      startTechnicalOutlinePolling(technicalOutlinePendingProjectId.value, true)
    } else {
      technicalGeneratingOutline.value = false
    }
  }
...
function openTechnicalFullGenerateDialog(action = 'GENERATE') {
  ...
  // 目标字数为空表示“AI 灵活生成”，不再阻止全文或失败章节生成。
...
async function startTechnicalFullGenerate(rewrite = false, skipConfirm = false, options = {}) {
  ...
  const projectId = selectedProject.value.id
  const timingTaskType = options?.retryFailedOnly ? 'RETRY_FAILED' : (rewrite ? 'REWRITE_FULL' : 'GENERATE_FULL')
  beginTechnicalGenerationTiming(projectId, timingTaskType)
  ...
  try {
    ...
    if (task?.id) {
      globalAiRunningTask.value = task
      syncTechnicalGenerationTimingFromTask(task, projectId)
      markTechnicalTaskPending(projectId, task.id, task)
      pollTechnicalGenerationTask(projectId, task.id, false)
    }
    await refreshWorkflow()
    await loadTechnicalSolution()
  } catch (e) {
    finishTechnicalGenerationTiming({ projectId, taskType: timingTaskType, status: 'FAILED' })
    fullGenerating.value = false
  }
}

function markTechnicalTaskPending(projectId, taskId, task = null) {
  if (!projectId || !taskId) return
  const normalizedProjectId = String(projectId)
  const normalizedTaskId = String(taskId)
  const sameTask = technicalTaskPending.projectId === normalizedProjectId && technicalTaskPending.taskId === normalizedTaskId
  technicalTaskPending.projectId = normalizedProjectId
  technicalTaskPending.taskId = normalizedTaskId
  ...
  fullGenerating.value = true
  if (task) syncTechnicalGenerationTimingFromTask(task, normalizedProjectId)
  ensureTechnicalGenerationTiming(normalizedProjectId, task?.taskType || technicalGenerationTiming.taskType || 'GENERATE_FULL', {
    taskId: normalizedTaskId,
    startedAt: parseGenerationTimestamp(task?.startTime || task?.createTime)
  })
  localStorage.setItem(TECH_TASK_PENDING_KEY, JSON.stringify({
    projectId: normalizedProjectId,
    taskId: normalizedTaskId,
    taskType: task?.taskType || technicalGenerationTiming.taskType || 'GENERATE_FULL',
    startedAt: Number(technicalGenerationTiming.startedAt || Date.now())
  }))
  if (!sameTask || !technicalTaskPoller.value?.active) startTechnicalTaskPolling(normalizedProjectId, normalizedTaskId)
}
...
function restoreTechnicalTaskPending() {
  ...
  try {
    ...
    if (data?.projectId && data?.taskId) {
      ...
      ensureTechnicalGenerationTiming(data.projectId, data.taskType || 'GENERATE_FULL', {
        taskId: data.taskId,
...
async function cancelCurrentTechnicalTask() {
  ...
  try {
    ...
    finishTechnicalGenerationTimingFromTask(canceled || task, projectId, 'CANCELED')
    globalAiRunningTask.value = null
...
async function pollTechnicalGenerationTask(projectId, taskId, silent = true) {
  ...
  try {
    ...
    syncTechnicalGenerationTimingFromTask(task, projectId, status)
    ...
    finishTechnicalGenerationTimingFromTask(task, projectId, status)
    ...
    fullGenerating.value = true
    if (status === 404) {
      finishTechnicalGenerationTiming({ projectId, taskId, status: 'FAILED' })
      clearTechnicalTaskPending(projectId, taskId)
      fullGenerating.value = false
      return
    }
...
function notifyTechnicalTaskTerminal(projectId, taskId, status, task, silent = true) {
  ...
  const elapsedSuffix = technicalGenerationTimingVisible.value ? `，本次用时 ${technicalGenerationElapsedText.value}` : ''
  const timedMessage = `${message}${elapsedSuffix}`
  if (status === 'FAILED' || status === 'TIMEOUT') ElMessage.error(timedMessage)
  else if (status === 'PARTIAL') ElMessage.warning(timedMessage)
  else if (status === 'CANCELED') ElMessage.warning(timedMessage)
  else ElMessage.success(timedMessage)
}
...
function isGenerationTimingActiveStatus(status) {
  return ['WAITING', 'RUNNING', 'OUTLINE_GENERATING'].includes(String(status || '').toUpperCase())
}
...
function persistTechnicalGenerationTiming() {
  if (!technicalGenerationTiming.projectId || !technicalGenerationTiming.startedAt) {
    localStorage.removeItem(TECH_GENERATION_TIMING_KEY)
    return
  }
  localStorage.setItem(TECH_GENERATION_TIMING_KEY, JSON.stringify({
    projectId: technicalGenerationTiming.projectId,
    taskId: technicalGenerationTiming.taskId,
    taskType: technicalGenerationTiming.taskType,
    status: technicalGenerationTiming.status,
    startedAt: technicalGenerationTiming.startedAt,
    finishedAt: technicalGenerationTiming.finishedAt
  }))
}

function restoreTechnicalGenerationTiming() {
  const raw = localStorage.getItem(TECH_GENERATION_TIMING_KEY)
  if (!raw) return
  try {
    const data = JSON.parse(raw)
    if (!data?.projectId || !Number(data?.startedAt || 0)) return
    Object.assign(technicalGenerationTiming, {
...
function beginTechnicalGenerationTiming(projectId, taskType, options = {}) {
  if (!projectId) return
  Object.assign(technicalGenerationTiming, {
    projectId: String(projectId),
    taskId: String(options.taskId || ''),
    taskType: String(taskType || 'GENERATE_FULL').toUpperCase(),
    status: String(options.status || (String(taskType || '').toUpperCase() === 'OUTLINE_GENERATE' ? 'OUTLINE_GENERATING' : 'WAITING')).toUpperCase(),
    startedAt: Number(options.startedAt || Date.now()),
    finishedAt: 0
  })
  technicalGenerationClock.value = Date.now()
  persistTechnicalGenerationTiming()
}

function ensureTechnicalGenerationTiming(projectId, taskType, options = {}) {
  if (!projectId) return
  const normalizedProjectId = String(projectId)
  const normalizedTaskId = String(options.taskId || '')
  const sameProject = String(technicalGenerationTiming.projectId || '') === normalizedProjectId
  const sameTask = !normalizedTaskId || !technicalGenerationTiming.taskId || String(technicalGenerationTiming.taskId) === normalizedTaskId
  if (!sameProject || !sameTask || !technicalGenerationTiming.startedAt || !isGenerationTimingActiveStatus(technicalGenerationTiming.status)) {
    beginTechnicalGenerationTiming(normalizedProjectId, taskType, options)
    return
  }
  if (normalizedTaskId) technicalGenerationTiming.taskId = normalizedTaskId
  if (taskType) technicalGenerationTiming.taskType = String(taskType).toUpperCase()
  if (options.status) technicalGenerationTiming.status = String(options.status).toUpperCase()
  persistTechnicalGenerationTiming()
}

function syncTechnicalGenerationTimingFromTask(task, projectId, normalizedStatus = '') {
  if (!task || !projectId) return
  const status = String(normalizedStatus || normalizeTechnicalTaskStatus(task) || task.status || 'RUNNING').toUpperCase()
  const parsedStart = parseGenerationTimestamp(task.startTime || task.createTime)
  ensureTechnicalGenerationTiming(projectId, task.taskType || 'GENERATE_FULL', {
    taskId: task.id,
    startedAt: parsedStart || Date.now(),
    status
  })
  technicalGenerationTiming.status = status
  if (task.id) technicalGenerationTiming.taskId = String(task.id)
  if (task.taskType) technicalGenerationTiming.taskType = String(task.taskType).toUpperCase()
  if (!isGenerationTimingActiveStatus(status)) {
    technicalGenerationTiming.finishedAt = parseGenerationTimestamp(task.finishTime || task.updateTime) || Date.now()
  }
  persistTechnicalGenerationTiming()
}

function finishTechnicalGenerationTiming({ projectId, taskId = '', taskType = '', status = 'SUCCESS', startedAt = 0, finishedAt = 0 } = {}) {
  if (!projectId) return
  const normalizedProjectId = String(projectId)
  const sameProject = String(technicalGenerationTiming.projectId || '') === normalizedProjectId
  const sameTask = !taskId || !technicalGenerationTiming.taskId || String(technicalGenerationTiming.taskId) === String(taskId)
  if (!sameProject || !sameTask || !technicalGenerationTiming.startedAt) {
    beginTechnicalGenerationTiming(normalizedProjectId, taskType || 'GENERATE_FULL', {
      taskId,
      startedAt: Number(startedAt || Date.now()),
      status
    })
  }
  if (taskId) technicalGenerationTiming.taskId = String(taskId)
  if (taskType) technicalGenerationTiming.taskType = String(taskType).toUpperCase()
  technicalGenerationTiming.status = String(status || 'SUCCESS').toUpperCase()
  technicalGenerationTiming.finishedAt = Number(finishedAt || Date.now())
  technicalGenerationClock.value = technicalGenerationTiming.finishedAt
  persistTechnicalGenerationTiming()
}

function finishTechnicalGenerationTimingFromTask(task, projectId, normalizedStatus = '') {
  if (!projectId) return
  finishTechnicalGenerationTiming({
    projectId,
    taskId: task?.id,
    taskType: task?.taskType || technicalGenerationTiming.taskType,
    status: normalizedStatus || task?.status || 'SUCCESS',
    startedAt: parseGenerationTimestamp(task?.startTime || task?.createTime),
    finishedAt: parseGenerationTimestamp(task?.finishTime || task?.updateTime)
  })
}

function syncTechnicalGenerationStateFromSolution(solution, projectId) {
  if (!solution || !projectId) return
  const status = String(solution.status || '').toUpperCase()
  const outlines = getTechnicalOutlinesFromSolution(solution)
  if (status === 'OUTLINE_GENERATING' && !outlines.length) {
    ensureTechnicalGenerationTiming(projectId, 'OUTLINE_GENERATE', {
      startedAt: parseGenerationTimestamp(solution.updateTime),
      status: 'OUTLINE_GENERATING'
    })
    markTechnicalOutlinePending(projectId)
    return
  }
  const runningTask = solution.runningTask
  if (runningTask?.id && isTechnicalTaskRunningStatus(runningTask.status)) {
    syncTechnicalGenerationTimingFromTask(runningTask, projectId)
    markTechnicalTaskPending(projectId, runningTask.id, runningTask)
  }
}
...
function markTechnicalOutlinePending(projectId) {
  ...
  const sameProject = String(technicalOutlinePendingProjectId.value || '') === id
  technicalOutlinePendingProjectId.value = id
  technicalGeneratingOutline.value = true
  ensureTechnicalGenerationTiming(id, 'OUTLINE_GENERATE', { status: 'OUTLINE_GENERATING' })
  localStorage.setItem(TECH_OUTLINE_PENDING_KEY, id)
  if (!sameProject || !technicalOutlinePoller.value?.active) startTechnicalOutlinePolling(id, false)
}
...
function restoreTechnicalOutlinePending() {
  ...
  technicalGeneratingOutline.value = true
  ensureTechnicalGenerationTiming(id, 'OUTLINE_GENERATE', { status: 'OUTLINE_GENERATING' })
  startTechnicalOutlinePolling(id, true)
}

function startTechnicalOutlinePolling(projectId, immediate = true) {
  if (!projectId) return
  technicalOutlinePoller.value?.stop()
  technicalOutlinePoller.value = createSerialPoller(
    async () => {
      await checkTechnicalOutlineReady(projectId, false)
      const stillPending = String(technicalOutlinePendingProjectId.value || '') === String(projectId || '')
      return stillPending ? true : { stop: true }
    },
    {
      interval: ({ elapsedMs }) => elapsedMs < 30000 ? 3000 : (elapsedMs < 180000 ? 5000 : 8000),
      maxBackoff: 30000,
      onError() {
  ...
  technicalOutlinePoller.value.start({ immediate })
}
...
async function checkTechnicalOutlineReady(projectId, silent = true) {
  ...
  try {
    ...
    if (!silent) {
      ElMessage.success(`技术方案目录已生成，本次用时 ${technicalGenerationElapsedText.value}，请点击“开始生成”或单章节“生成”后设置目标字数`)
    }
    return true
  } catch (e) {
    ...
    if (technicalOutlinePollErrorCount.value === 6 && String(selectedProject.value?.id || '') === String(projectId || '')) {
      ElMessage.warning('目录状态查询持续失败，系统已降低刷新频率并会继续自动检查，无需手工刷新页面')
    }
    return false
  } finally {
...
async function onTechnicalNodeWordChange({ node, value }) {
  if (!node?.id) return
  const targetWordCount = normalizeTargetWordCount(value)
  await updateBidProjectTechnicalOutlineWordCount(selectedProject.value?.id, node.id, targetWordCount)
  node.targetWordCount = targetWordCount
  node.wordCount = targetWordCount
  await reloadTechnicalAfterOutlineEdit('字数已保存')
}

async function onTechnicalBatchWord({ node, value }) {
  if (!node?.id) return
  await batchUpdateBidProjectTechnicalOutlineWordCount(selectedProject.value?.id, node.id, normalizeTargetWordCount(value))
  await reloadTechnicalAfterOutlineEdit('下级章节字数已批量修改')
}

async function onTechnicalAllBatchWord() {
  if (!selectedProject.value?.id || technicalAllWordCount.value === '') return
  const targetWordCount = normalizeTargetWordCount(technicalAllWordCount.value)
  const label = targetWordCount == null ? '不指定字数，由 AI 灵活生成' : `统一设置为 ${targetWordCount} 字`
  await ElMessageBox.confirm(`确定将全部未生成的末级章节${label}吗？`, '整份方案统一设置', { type: 'warning' })
  technicalAllWordUpdating.value = true
  try {
    await batchUpdateBidProjectTechnicalAllOutlineWordCount(selectedProject.value.id, targetWordCount)
    await reloadTechnicalAfterOutlineEdit(targetWordCount == null ? '全部章节已改为 AI 灵活生成' : '全部章节字数已统一修改')
  } finally {
    technicalAllWordUpdating.value = false
  }
}
...
function openTechnicalSectionDialog(node) {
  ...
  const targetWordCount = normalizeTargetWordCount(node.targetWordCount ?? node.wordCount ?? node.section?.targetWordCount)
...
async function generateTechnicalSection() {
  ...
  const projectId = selectedProject.value?.id
  beginTechnicalGenerationTiming(projectId, 'GENERATE_SECTION', { taskId: sectionNode.value.id, status: 'RUNNING' })
  ...
  try {
    ...
    finishTechnicalGenerationTiming({ projectId, taskId: sectionNode.value.id, taskType: 'GENERATE_SECTION', status: 'SUCCESS' })
    ElMessage.success(`本段生成完成，用时 ${technicalGenerationElapsedText.value}`)
  } catch (e) {
    finishTechnicalGenerationTiming({ projectId, taskId: sectionNode.value.id, taskType: 'GENERATE_SECTION', status: 'FAILED' })
    throw e
  } finally {
    if (!generatedSuccessfully && isGenerationTimingActiveStatus(technicalGenerationTiming.status)) {
      finishTechnicalGenerationTiming({ projectId, taskId: sectionNode.value.id, taskType: 'GENERATE_SECTION', status: 'FAILED' })
    }
