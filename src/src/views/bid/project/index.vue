  ...
  <div class="page">
    ...
    >
      <el-form ref="projectFormRef" :model="projectForm" :rules="projectRules" label-width="130px">
        ...
        <el-form-item label="招标单位" prop="clientName">
        ...
        <el-form-item label="项目负责人" prop="ownerUserId">
          ...
          >
            <el-option
              ...
              :key="item.id"
              :label="ownerUserLabel(item)"
              :value="item.id"
            />
          </el-select>
          <div class="form-tip">生成前检查会校验负责人；企业用户默认可选择本企业用户。</div>
        </el-form-item>
...
const projectRules = computed(() => {
  const rules = {
    ...
    clientName: [{ required: true, message: '请输入招标单位', trigger: 'blur' }],
    bidderName: [{ required: true, message: '请输入投标单位', trigger: 'blur' }],
...
onMounted(async () => {
  ...
  await loadOwnerUserOptions(auth.user?.enterpriseId || undefined)
...
async function selectProject(row) {
  ...
  if (selectedProject.value?.enterpriseId) {
    ...
    await loadOwnerUserOptions(selectedProject.value.enterpriseId)
  } else {
...
function resetProjectForm(row = {}) {
  ...
  projectForm.ownerUserId = row.ownerUserId || (!canManagePlatform.value ? auth.user?.id || null : null)
...
async function openCreateProject() {
  ...
  await loadOwnerUserOptions(projectForm.enterpriseId || undefined)
  if (!projectForm.ownerUserId && ownerUserOptions.value.length === 1) {
    projectForm.ownerUserId = ownerUserOptions.value[0].id
  }
...
async function openEditProject(row) {
  ...
  await loadOwnerUserOptions(detail.enterpriseId || undefined)
  projectDialog.visible = true
}
...
async function onProjectEnterpriseChange(value) {
  ...
  projectForm.ownerUserId = null
  await loadKnowledgeOptions(value || undefined)
  await loadBidTemplateOptions(value || undefined)
  await loadOwnerUserOptions(value || undefined)
  if (ownerUserOptions.value.length === 1) {
    projectForm.ownerUserId = ownerUserOptions.value[0].id
  }
...
function ownerUserLabel(item) {
  if (!item) return ''
  const name = item.fullName || item.username || item.phone || `用户 #${item.id}`
